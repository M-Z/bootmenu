$(document).ready(function() {
    ...
    $("select").change(function() {
        var value = $("option:selected", this).attr('currentValue');

        // PHP Request for the next menu items
        $.ajax({
            type: 'POST',
            url: 'dynamics/search.php',
            ...
            success: function(result) {
                // on success
                if (result != 'Failure') {
                    $(".menu").slice(1).each(function() {
                        $(this).hide();
                    });

                    var new_ = $('.searchin').append(result);
                } else alert("Query failed dude!");
            },
            error: function() {
                // on failure
    ...
    $("#selectMenu").bootmenu({
        ...
        listName: "Food",
        callback: function(encryptedValue) {
    ...
    $.fn.bootmenu = function(options, callback) {
        ...
        var select = $('ul#' + settings.listName + ' a').on("click", function() {
            ...
            if (typeof settings.callback == 'function')
                settings.callback.apply(this, [btoa(value)]); // callback the requested function and return the encrypted value
        });
